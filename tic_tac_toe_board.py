...
class TicTacToeBoard:
    ...
    def unmark(self, row, col):
        """
        Remove the mark from a given square and switch the next player.

        Given a row and column index, clear the square of its current mark and
        revert the next move to the previous player. Raise an error if the
        square is already blank.

        Args:
            row: An int representing the index of the row of the board to unmark.
            col: An int representing the index of the column of the board to
                unmark.

        Raises:
            ValueError: if the requested square is already blank.
        """
        if self.get_square(row, col) == self.blank_mark:
            raise ValueError
        self._board[row][col] = self.blank_mark
        self._flip_next_move()
    ...
    def check_win(self, player):
        ...
        """
        return (
            self._check_row_win(player)
            or self._check_col_win(player)
    ...
    def game_over(self):
        ...
        Returns True if either player has won or if all squares are filled,
        resulting in a tie.
        ...
        if self.check_win(self.player_1_mark) or self.check_win(self.player_2_mark):
            return True

        for row in self._board:
            if self.blank_mark in row:
                return False
